...
/**
 * calculates the 2d chern number via the brillouin zone boundary
...
requires is_mat<t_mat> && is_vec<t_vec> && is_vec<t_vec_real> && is_complex<t_cplx>
{
	...
	auto int_boundary = [get_evecs, bz, dim1, dim2,
		delta_diff, delta_int, &nums](
			int dim, t_vec_real& Q, t_real int_sign = 1.)
	{
		for(t_real x = -bz; x < bz; x += delta_int, Q[dim] += delta_int)
		{
			std::vector<t_vec> conns = berry_connection<
			...
			{
				nums[band] += conns[band][dim1] * delta_int * int_sign;
				nums[band] -= conns[band][dim2] * delta_int * int_sign;
			}
		}
	};
...
/**
 * calculates the 2d chern number via the brillouin zone area
 * @see equ. 9 in https://doi.org/10.1146/annurev-conmatphys-031620-104715
 * @see https://en.wikipedia.org/wiki/Berry_connection_and_curvature
 */
template<class t_mat, class t_vec, class t_vec_real,
	typename t_cplx = typename t_vec::value_type,
	typename t_real = typename t_cplx::value_type>
std::vector<t_cplx> chern_numbers_2d_area(
	const std::function<t_mat(const t_vec_real& Q)>& get_evecs,
	const t_vec_real& G, t_real bz = 0.5,  // brillouin zone boundary
	t_real delta_diff = std::numeric_limits<t_real>::epsilon(),
	t_real delta_int = std::numeric_limits<t_real>::epsilon(),
...
requires is_mat<t_mat> && is_vec<t_vec> && is_vec<t_vec_real> && is_complex<t_cplx>
{
	using t_size = decltype(t_vec_real{}.size());
	std::vector<t_cplx> nums;  // chern numbers per band

	t_vec_real Q = G;
	for(t_real x = -bz; x < bz; x += delta_int, Q[dim1] += delta_int)
	{
		for(t_real y = -bz; y < bz; y += delta_int, Q[dim2] += delta_int)
		{
			std::vector<t_cplx> curvs = berry_curvature_2d<
				t_mat, t_vec, t_vec_real, t_cplx, t_real>(
					get_evecs, Q, delta_diff, dim1, dim2);

			// initialise by resetting chern numbers to zeros
			if(!nums.size())
				nums.resize(curvs.size(), t_cplx{});

			// numerically integrate the brillouin zone area for each band
			for(t_size band = 0; band < curvs.size(); ++band)
				nums[band] += curvs[band] * delta_int * delta_int;
		}
	}
