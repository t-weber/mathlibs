 ...
 *
 * g++ -std=c++20 -I../libs -Wall -Wextra -Weffc++ -o quat quat.cpp
 */
...
void quat_tests()
{
	...
	t_quat q2 = m::rotation<t_quat, t_vec>(axis2, angle2);
	...
	std::cout << "\ncalculating quaternion and rotation matrix representing the same rotation\n";
	t_vec vec_from = m::create<t_vec>({1, 2, 3});
	t_vec vec_to = m::create<t_vec>({5, 4, 3});
	t_quat q3 = m::rotation<t_quat, t_vec>(vec_from, vec_to);
	t_mat rot3 = m::rotation<t_mat, t_vec>(vec_from, vec_to);

	std::cout << "q3 = ";
	m_ops::operator<<(std::cout, q3) << std::endl;

	// convert quaternion to SO(3) rotation matrix
	t_mat mat3_so3 = m::to_so3<t_quat, t_vec, t_mat>(q3);
	std::cout << "so3:  ";
	m_ops::operator<<(std::cout, mat3_so3) << std::endl;

	std::cout << "rot3: ";
	m_ops::operator<<(std::cout, rot3) << std::endl;
